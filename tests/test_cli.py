# -*- coding: utf-8 -*-
"""CLI smoke tests."""
from __future__ import annotations

import json
import subprocess
import sys


def test_cli_convert_json() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "mystilink_lunar",
            "convert",
            "--date",
            "1993-09-28",
            "--time",
            "13:21",
            "--timezone",
            "Asia/Shanghai",
            "--json",
        ],
        check=False,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stderr
    data = json.loads(proc.stdout)
    assert data["lunar"]["month"] == 8
    assert data["lunar"]["day"] == 13
    assert data["ganzhi"]["year"]["text"] == "癸酉"
    assert data["ganzhi"]["month"]["text"] == "辛酉"
    assert data["ganzhi"]["day"]["text"] == "壬子"
    assert data["ganzhi"]["hour"]["text"] == "丁未"


def test_cli_convert_explain() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "mystilink_lunar",
            "convert",
            "--date",
            "1993-09-28",
            "--time",
            "13:21",
            "--timezone",
            "Asia/Shanghai",
            "--profile",
            "bazi",
            "--json",
            "--explain",
        ],
        check=False,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stderr
    data = json.loads(proc.stdout)
    assert data["ganzhi"]["hour"]["text"] == "丁未"
    assert data["ganzhi_explain"]["year"]["boundary"] == "lichun_exact"
    assert data["rules"]["day_boundary"] == "zi_start"
    assert data["solar_term"]["previous"]["id"] == "qiufen"
    assert data["solar_term"]["next"]["id"] == "hanlu"


def test_cli_solar_term_json() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "mystilink_lunar",
            "solar-term",
            "--name",
            "lichun",
            "--year",
            "2026",
            "--timezone",
            "Asia/Shanghai",
            "--json",
        ],
        check=False,
        capture_output=True,
        text=True,
    )
    assert proc.returncode == 0, proc.stderr
    data = json.loads(proc.stdout)
    assert data["id"] == "lichun"
    assert data["datetime"].startswith("2026-02-04T04:01:")



def test_cli_requires_timezone() -> None:
    proc = subprocess.run(
        [
            sys.executable,
            "-m",
            "mystilink_lunar",
            "convert",
            "--date",
            "1993-09-28",
        ],
        check=False,
        capture_output=True,
        text=True,
    )
    assert proc.returncode != 0
